...
def break_tweet_timeline(tolerance):
    ...
    list_tweet_ids = generate_tweet_timestamp_id(list_tweets[0],
                                                 list_tweets[1],
                                                 twitter_object, list_tweets,
                                                 tolerance)
...
def write_data_points(list_tweets):
    ...
    list_tweet_timeline = []
    list_tweet_timestamps, list_tweet_ids = zip(
        *sorted(zip(list_tweet_timestamps, list_tweet_ids)))
    for i in range(0, len(list_tweet_ids)):
        list_tweet_timeline.append(
            [list_tweet_ids[i], list_tweet_timestamps[i]])
    with open(os.path.join(data_directory, "TweetTimelineList.txt"),
              "w") as file_tweets:
        file_tweets.write(str(list_tweet_timeline))
    with open(os.path.join(data_directory, "TweetTimeline.txt"),
              "w") as file_tweet_timeline:
        for i in range(0, len(list_tweet_ids)):
            file_tweet_timeline.write(
                str(list_tweet_ids[i]) + "," + str(list_tweet_timestamps[i]) +
                "," +
                str(datetime.utcfromtimestamp(list_tweet_timestamps[i])) +
                "\n")
...
def generate_tweet_timestamp_id(start, end, twitter_object, list_tweets,
                                tolerance):
    if abs(end["Timestamp"] - start["Timestamp"]
           ) <= tolerance or end["Timestamp"] < start["Timestamp"]:
        return list_tweets
    mid = int((start["TweetId"] + end["TweetId"]) / 2)
    tweet_id = get_current_tweet_id(mid, int(start["TweetId"]),
                                    int(end["TweetId"]))
    if tweet_id > 0:
        ...
        generate_tweet_timestamp_id(start, mid, twitter_object, list_tweets,
                                    tolerance)
        generate_tweet_timestamp_id(mid, end, twitter_object, list_tweets,
                                    tolerance)
    return list_tweets
...
def get_tweet_timestamp(tweet_id, twitter_object):
    try:
        ...
        tweet_date_time = datetime.strptime(twitter_response.created_at,
                                            "%a %b %d %H:%M:%S %z %Y")
        ...
        if response.status_code == 200:
            ...
            if "status/" in response.headers['location']:
                ...
                if redirect_response.status_code == 200:
                    return int(
                        str(response.headers['location']).split("/")[-1])
                else:
                    with open(os.path.join(data_directory, "WeirdUrls.txt"),
                              "a+") as file_urls:
                        file_urls.write(
                            str(current_tweet_id) + "," +
...
def create_twitter_instance():
    api = twitter.Api(
        consumer_key='5Q3CFnvq02nKj6kI9gRpGNHXH',
        ...
        if tid < int(prev_line_parts[0]):
            ...
            for line in file_tweet_timeline:
                ...
                if int(prev_line_parts[0]) < tid < int(line_parts[0]):
                    estimated_timestamp = round(
                        int(prev_line_parts[1]) +
                        (((tid - int(prev_line_parts[0])) /
                          (int(line_parts[0]) - int(prev_line_parts[0]))) *
                         (int(line_parts[1]) - int(prev_line_parts[1]))))
...
def create_test_set(start_tweet_id,
                    end_tweet_id,
                    data_points=0,
                    data_interval=0):
    list_current_test_id = []
    data_directory = os.path.join(os.path.dirname(__file__), '..', "data")
    with open(os.path.join(data_directory, "testset.txt"),
              "w") as file_test_set:
        if data_interval > 0:
            with open(os.path.join(data_directory, "TweetTimeline.txt"),
                      "r") as file_tweets:
                ...
                for line in file_tweets:
                    line_parts = line.rstrip().split(",")
                    if int(line_parts[1]) < week_timestamp + (data_interval *
                                                              24 * 60 * 60):
            ...
            for i in range(1, len(list_interval_tweet_ids)):
                find_tweet_ids(list_interval_tweet_ids[i - 1],
                               list_interval_tweet_ids[i], data_points,
                               list_current_test_id, file_test_set)
        else:
            find_tweet_ids(start_tweet_id, end_tweet_id, data_points,
                           list_current_test_id, file_test_set)
...
def find_tweet_ids(start_tweet_id, end_tweet_id, data_points,
                   list_current_test_id, file_test_set):
    sample_twitter_url = "https://twitter.com/jack/status/"
    points = 0
    twitter_object = create_twitter_instance()
    ...
    while points < data_points and count < (data_points * 5):
        ...
        if tweet_status and current_tweet_id not in list_current_test_id:
            ...
            random_tweet_timestamp = get_tweet_timestamp(
                current_tweet_id, twitter_object)
            file_test_set.write(
                str(current_tweet_id) + "," + str(random_tweet_timestamp) +
                "\n")
            points += 1
        count += 1
    return
...
def find_estimate_error():
    ...
    print("Error rate: " + str(hour_error) + ":" + str(minute_error) + ":" +
          str(error_rate))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=
        'Create a pre-Snowflake Tweet Id dataset based on threshold value, Find timestamp of any pre or post Snowflake Tweet id ,Create Pre-Snowflake Twitter test dataset and check errors on them'
    )
    ...
    group.add_argument(
        ...
        default=None,
        help=
        "Create test set with argument of start, end Tweet Id, and no. of data points"
    )
    group.add_argument(
        '-d',
        dest='dataset',
        ...
        help="Create a dataset with argument of theshold value in seconds")
    group.add_argument('-e',
                       dest='errortest',
                       action='store_true',
                       help="Check error on pre-Snowflake ids")
    group.add_argument('-t',
                       dest='timestamp',
                       type=int,
                       help="Find timestamp of any tweet id")
    args = parser.parse_args()
    if args.testset:
        start_tweet_id, end_tweet_id, data_points, data_interval = fix_test_set_arguments(
            args.testset)
        create_test_set(start_tweet_id, end_tweet_id, data_points,
                        data_interval)
    elif args.dataset:
